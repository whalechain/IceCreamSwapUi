...
const ContributeModal: React.FC<React.PropsWithChildren<Props>> = ({
  ...
  const label = currency === bscTokens.cake ? t('Max. ICE entry') : t('Max. token entry')
  ...
  const basicTooltipContent = t(
    'For the private sale, each eligible participant will be able to commit any amount of ICE up to the maximum commit limit, which is published along with the IFO voting proposal.',
  )

  const unlimitedToolipContent = (
    <Box>
      <Text display="inline">{t('For the public sale, Max ICE entry is capped by')} </Text>
      <Text bold display="inline">
        {t('the number of iICE.')}{' '}
      </Text>
      <Text display="inline">
        {t('Lock more ICE for longer durations to increase the maximum number of ICE you can commit to the sale.')}
      </Text>
    </Box>
  )
      ...
      <ModalBody maxWidth="360px">
        <Box p="2px">
          ...
          <Text color="textSubtle" fontSize="12px" mb="24px">
            {t(
              'If you don’t commit enough ICE, you may not receive a meaningful amount of IFO tokens, or you may not receive any IFO tokens at all.',
            )}
            <Link
              fontSize="12px"
              display="inline"
              href="https://docs.icecreamswap.com/products/ifo-initial-farm-offering"
              external
            >
              {t('Read more')}
            </Link>
