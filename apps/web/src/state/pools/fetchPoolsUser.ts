...
export const fetchPoolsAllowance = async (account, chainId) => {
  ...
  return fromPairs(
    nonBnbPools
      .filter((poolConfig) => chainId in poolConfig.contractAddress)
      .map((pool, index) => [pool.sousId, new BigNumber(allowances[index]).toJSON()]),
  )
}
...
export const fetchUserBalances = async (account, chainId) => {
  // Non BNB pools
  const tokens = uniq(
    nonBnbPools.filter((poolConfig) => chainId in poolConfig.contractAddress).map((pool) => pool.stakingToken.address),
  )
  ...
  const bnbBalances = fromPairs(
    bnbPools.filter((poolConfig) => chainId in poolConfig.contractAddress).map((pool) => [pool.sousId, bnbBalanceJson]),
  )
...
export const fetchUserStakeBalances = async (account, chainId) => {
  ...
  return fromPairs(
    nonMasterPools
      .filter((poolConfig) => chainId in poolConfig.contractAddress)
      .map((pool, index) => [pool.sousId, new BigNumber(userInfo[index].amount._hex).toJSON()]),
  )
}
...
export const fetchUserPendingRewards = async (account, chainId) => {
  ...
  return fromPairs(
    nonMasterPools
      .filter((poolConfig) => chainId in poolConfig.contractAddress)
      .map((pool, index) => [pool.sousId, new BigNumber(res[index]).toJSON()]),
  )
}
