...
import { GRAPH_API_PREDICTION_BNB, GRAPH_API_PREDICTION_ICE } from 'config/constants/endpoints'
...
import { transformBetResponseICE, transformUserResponseICE } from './cakeTransformers'
...
export const transformBetResponse = (tokenSymbol) =>
  tokenSymbol === 'ICE' ? transformBetResponseICE : transformBetResponseBNB

export const transformUserResponse = (tokenSymbol) =>
  tokenSymbol === 'ICE' ? transformUserResponseICE : transformUserResponseBNB
...
export const getTotalWon = async (): Promise<{ totalWonBNB: number; totalWonICE: number }> => {
  const [{ market: BNBMarket, market: ICEMarket }] = await Promise.all([
    request(
  ...
  const totalWonICE = getTotalWonMarket(ICEMarket, 'ICE')

  return { totalWonBNB, totalWonICE }
}
...
export const getBetHistory = async (
  ...
  api: string,
  tokenSymbol: string,
): Promise<Array<BetResponseBNB | BetResponseICE>> => {
